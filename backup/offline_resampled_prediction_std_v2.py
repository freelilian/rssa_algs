...
def calculate_std(row):
    ## row only contains prediction values (including nan/zero), so make sure the item ids are not listed in the first column in the df
    pass
...
if __name__ == "__main__":
    ...
    for user in users[0:100]:
        ...
        for i in range(numResampledModels):
            ...
            items_in_sample = algo_resampled.item_index_.to_numpy()
                # only include items resampled in this current model
            resampled_preds = algo_resampled.predict_for_user(user, items_in_sample)
